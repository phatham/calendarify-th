...
export class Helpers {
  ...
  render() {
    ...
    <div class="calendarify__quick-actions ${
      !this.quickActions ? 'd-none' : ''
    }">
      <button data-action="today">${
        this.locale.lang?.ui?.quickActions?.today
      }</button>
      <button data-action="tomorrow">${
        this.locale.lang?.ui?.quickActions?.tomorrow
      }</button>
      <button data-action="in-2-days">${
        this.locale.lang?.ui?.quickActions?.inTwoDays
      }</button>
    </div>
    ...
    <div class="calendarify__calendar">
      ...
      <ul class="dates-wrapper calendarify-wrapper ">
          ${this.dates
            .map((date) => {
              ...
              } type="button" class="date-button ${this.getHolidayClass({
                date: date.date,
      ...
      <ul class="months-wrapper calendarify-wrapper d-none">
        ${this.months
          .map((month, index) => {
            const monthShort =
              this.locale.lang?.shortMonths == null
                ? this.locale.lang?.months![index].slice(0, 3)
                : this.locale.lang?.shortMonths[index];
            const nowMonthOnly = moment().format('M');
            return `<li><button class="${
              month == nowMonthOnly ? 'active' : ''
            }" data-date="${month}" type="button">${monthShort}</button></li>`;
          })
          .join('')}
      </ul>
