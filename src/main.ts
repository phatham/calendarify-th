...
export default class Calendarify {
  ...
  constructor(inputSelector: string, options?: Partial<Calendarify>) {
    ...
    rootElement.style.setProperty(
      '--accentColor',
      this.options.accentColor ?? this.accentColor
    );
    ...
    this.locale = {
      ...
      lang: {
        ...
        ui: {
          quickActions: {
            today:
              this.options?.locale?.lang?.ui?.quickActions?.today ?? 'Today',
            tomorrow:
              this.options?.locale?.lang?.ui?.quickActions?.tomorrow ??
              'Tomorrow',
            inTwoDays:
              this.options?.locale?.lang?.ui?.quickActions?.inTwoDays ??
              'In 2 Days',
          },
        },
        months: this.options?.locale?.lang?.months ?? moment.months(),
        shortMonths: this.options?.locale?.lang?.shortMonths,
        weekdays: this.options?.locale?.lang?.weekdays ?? moment.weekdays(),
      },
    };

    this.startDate =
      this.options.startDate ?? moment().format(this._systemFormat);
    this._date = this.options.startDate ?? this.startDate;

    this._months = [];
    for (let i = 0; i < 12; i++) {
      this._months.push(String(i + 1));
    }
    ...
    this._helpers = new Helpers({
      container: document.querySelector(inputSelector)
        ?.parentElement as HTMLAreaElement,
      dates: this._dates,
      months: this._months,
      years: this._years,
    ...
    this._datepickerInput = document.querySelector(
      inputSelector
    ) as HTMLInputElement;
  ...
  public init() {
    ...
    this._quickButtons.forEach((button) =>
      button.addEventListener('click', this.quickAction.bind(this))
    );
  }

  private stylingContainer() {
    if (this.options?.isDark)
      this._container.setAttribute('data-theme', 'dark');
    ...
    if (this.options?.customClass) {
      this.customClass.forEach((customClass) =>
        this._container.classList.add(customClass)
      );
    }
  ...
  private quickAction(event: Event) {
    ...
    switch (data) {
      case 'today':
        ...
        break;
      case 'tomorrow':
        this._date = moment(this.startDate)
          .add(1, 'days')
  ...
  private hideOnOutsideClick(event: Event) {
    ...
    if (
      !targetElement.closest(this._inputSelector) &&
      !targetElement.closest('.calendarify')
    ) {
  ...
  private showMonths() {
    ...
    const monthButtons = this._monthsWrapperEl.querySelectorAll(
      'button'
    ) as NodeListOf<HTMLButtonElement>;
    monthButtons.forEach((button) =>
      button.addEventListener('click', (event) =>
        this.changeMonth(event, monthButtons)
      )
    );
  ...
  private showYears() {
    ...
    this._yearsWrapperEl.innerHTML = `${this._years
      .map((year) => {
        return `<li><button class="${
          year == this._nowYear ? 'active' : ''
        }" data-date="${year}" type="button">${
          parseInt(year) + this.yearOffset
        }</button></li>`;
      })
      .join('')}`;

    const yearButtons = this._yearsWrapperEl.querySelectorAll(
      'button'
    ) as NodeListOf<HTMLButtonElement>;
    yearButtons.forEach((button) =>
      button.addEventListener('click', this.changeYear.bind(this))
    );
  ...
  private changeYear(event: Event) {
    ...
    this._date = moment(`${year}-${month}-${this._nowDay}`).format(
      this._systemFormat
    );
    this._nowYear = year;
  ...
  private changeState() {
    ...
    const formattedYear =
      parseInt(moment(this._nowMonth).format('YYYY')) + this.yearOffset;
    this._expandButton.textContent = `${
      this.locale.lang?.months![+formattedMonth - 1]
    } ${formattedYear}`;

    switch (this._expandedMode) {
      case 'years':
        this._yearRangeButton.textContent = `${
          parseInt(this._years[0]) + this.yearOffset
        } - ${parseInt(this._years[this._years.length - 1]) + this.yearOffset}`;
        break;
      default:
        this._yearRangeButton.textContent = (
          parseInt(moment(this._date).format('YYYY')) + this.yearOffset
        ).toString();
        break;
    }
    ...
    this._dateButtons = this._datesWrapperEl.querySelectorAll(
      '.date-button'
    ) as NodeListOf<HTMLButtonElement>;
    this._dateButtons.forEach((button) =>
      button.addEventListener('click', this.setDate.bind(this))
    );

    if (this._expandedMode == 'months') {
      const isFirstRowDisabled = this._dates
        .slice(0, 7)
        .every((date) => date.disabled);
      if (isFirstRowDisabled)
        for (let i = 0; i < 7; i++) {
          this._dateButtons[i].parentElement?.classList.add('d-none');
        }
    }
  }
  ...
  private loopDaysMonths() {
    ...
    const firstWeekdayOfTheMonth = moment(
      new Date(this._nowMonth)
    ).isoWeekday();
    const daysInBeforeMonth = moment(this._date)
      .subtract(1, 'months')
      .daysInMonth();
    ...
    for (let i = 1; i <= firstWeekdayOfTheMonth; i++) {
      this._dates.unshift({
        disabled: true,
        date: String(daysInBeforeMonth + 1 - i),
      });
    }
    // Loop days of the month
    for (let i = 1; i <= daysInMonth; i++) {
      this._dates.push({ disabled: false, date: String(i) });
    }
  }
  ...
  private renderDates() {
    this._datesWrapperEl.innerHTML = `${this._dates
      .map((date) => {
        ...
        } type="button" class="date-button ${this._helpers.getHolidayClass({
          date: date.date,
          nowMonth: this._nowMonth,
        })} ${this._nowDay == String(date.date) ? 'active' : ''}">${
          date.date
        }</button></li>`;
      })
      .join('')}`;
  ...
  private setDate(event: Event) {
    ...
    this._date = `${moment(`${this._nowMonth}-${this._nowDay}`).format(
      this._systemFormat
    )}`;
  ...
  private prevMonth() {
    if (this._isExpanded) {
      switch (this._expandedMode) {
        case 'months':
          this._date = moment(`${this._nowMonth}-${this._nowDay}`)
            .subtract(1, 'years')
            .format(this._systemFormat);
          break;
        default:
